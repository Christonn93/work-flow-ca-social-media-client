...
describe('Social Media App: Authenticated user', () => {
  beforeEach(() => {
    ...
    cy.get("input[type='password']")
      .should('exist')
      .type(`cypresstestpassword{enter}`);
    cy.wait(1000);
  });
  ...
  it('CAN make a new post', () => {
    cy.visit('http://127.0.0.1:8080/post.html');
    cy.get('form.create-post-form')
      .should('exist')
      .within(() => {
        cy.get("input[name='title']")
          .should('exist')
          .type(`Cypress test post title`);
        cy.get("textarea[name='body']")
          .should('exist')
          .type(`Cypress test post body`);
        cy.get("input[name='tags']")
          .should('exist')
          .type(`Cypress, automated, testing`);
